    ...
    for(int v = 0; v < 8; ++v)
    {
        for(int u = 0; u < 8; ++u)
        {
            for(int y= 0; y < 8; ++y)
            {
                for(int x = 0; x < 8; ++x)
                {
                    cosParam1[x * u] = cos((((2.0 * x) + 1) * u * M_PI) / 16.0);
                    cosParam2[y * v] = cos((((2.0 * y) + 1) * v * M_PI) / 16.0);
                }
            }
        }
    } 
    ...
    for(y = 0; y < 8; ++y)
    {
        for(x = 0; x < 8; ++x)
        {
            ...
            for(v = 0; v < 8; ++v)
            {
                for(u = 0; u < 8; ++u)
                {
                    sum += cArray[u] * cArray[v] * dctBlocks[block][v * u] * dctCosParam1[x * u] * dctCosParam2[y * v];
                    //sum += cArray[u] * cArray[v] * dctBlocks[block][v * u] * cos(v * M_PI * (2.0 * y + 1) / 16.0) * cos(u * M_PI * (2.0 * x + 1) / 16.0);
                }
            }
    
            blocks[block][y * x] =  0.25 * sum;
        }
    } 
    ...
    for(v= 0; v < 8; ++v)
    {
        for(u = 0; u < 8; ++u)
        {
            ...
            for(y = 0; y < 8; ++y)
            {
                for(x = 0; x< 8; ++x)
                {
                    sum += blocks[block][y * x] * dctCosParam1[x * u] * dctCosParam2[y * v];
                    //sum += blocks[block][y * x] * cos(v * M_PI * (2.0 * y + 1) / 16.0) * cos(u * M_PI * (2.0 * x + 1) / 16.0);
                }
            }
    
            dctBlocks[block][u * v] =  0.25 * cArray[u] * cArray[v] * sum;
        }
    } 
...
void BM3D::BM3D_2DiDCT()
{
    ...
    dim3 threadsPerBlock(1);
    dim3 numBlocks(BM3D::context.nbBlocks);
    iDCT2D8x8<<<numBlocks,threadsPerBlock>>>(BM3D::context.deviceBlocks2, BM3D::context.deviceBlocksDCT, BM3D::context.idctCosParam1, BM3D::context.idctCosParam2, BM3D::context.cArray);
    cudaThreadSynchronize ();
...
void BM3D::BM3D_2DDCT()
{
    ...
    dim3 threadsPerBlock(1);
    dim3 numBlocks(BM3D::context.nbBlocks);
    DCT2D8x8<<<numBlocks,threadsPerBlock>>>(BM3D::context.deviceBlocks2, BM3D::context.deviceBlocksDCT, BM3D::context.dctCosParam1, BM3D::context.dctCosParam2, BM3D::context.cArray);
    cudaThreadSynchronize ();
