...
//    Timer::startCuda();
    ...
    gpuErrchk(cudaMalloc(&BM3D::context.blockMap, BM3D::context.nbBlocks * 4 * sizeof(int))); //store x;y;vX;vY of each block (Block Map)
...
//    Timer::addCuda("Cuda initialization");
    ...
    printf("\n\tSize Block Map array = %f Mb", (BM3D::context.nbBlocks * 4 * sizeof(int) /1024.00 / 1024.00));  
...
__global__ void CreateBlocks_Zone(int* blockMap, int blockIndexOffset, int offsetX, int offsetY, int vX, int vY)
{
    ...
    int blockMapIndex = (block << 2);
    
    int xImg = offsetX + (vX * (x << 1));
    int yImg = offsetY + (vY * (y << 1));
    ...
    //printf("\nblock = %d, xImg = %d, yImg = %d, x = %d, y = %d, vx = %d, vy = %d, blockMapIndex = %d", block, blockMap[blockMapIndex], blockMap[blockMapIndex+1], x, y, blockMap[blockMapIndex+2], blockMap[blockMapIndex+3], blockMapIndex); 
}
