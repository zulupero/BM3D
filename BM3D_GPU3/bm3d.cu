...
void BM3D::BM3D_Initialize(BM3D::SourceImage img, BM3D::SourceImage imgOrig, int width, int height, int pHard, int hardLimit, int wienLimit, double hardThreshold, int sigma, int windowSize, int stepInWindow, bool debug)
{
    ...
    BM3D::context.stepInWindow = stepInWindow;
    ...
    BM3D::context.nbBlocksPerWindow = int(windowSize/stepInWindow) * int(windowSize/stepInWindow);
    ...
    BM3D::context.widthBlocksWindow = int(windowSize/stepInWindow);
...
{
    int block = (blockIdx.y * size) + blockIdx.z;
    int windowMapRefIndex = block * windowSize;
    int refX = windowMap[windowMapRefIndex];
    int refY = windowMap[windowMapRefIndex+1];    

    int windowMapBlockIndex = windowMapRefIndex + 4 + blockIdx.x * 4;
    int blockX = windowMap[windowMapBlockIndex];
    int blockY = windowMap[windowMapBlockIndex+1];
    ...
    for(int y=0; y<8; ++y)
    {
        for(int x=0; x<8; ++x)
        {   
            float valueRef = image[(refY + y) * width + refX + x];
            float valueBlock = image[(blockY + y) * width + blockX + x];
            distance += (valueRef - valueBlock) * (valueRef - valueBlock);
            //if(block == 1000) printf("\nvalue ref = %f,  value block = %f, blockY = %d, blockX = %d", valueRef, valueBlock, blockY, blockX);
        }
    }
    
    windowMap[windowMapBlockIndex+2] = int(distance);
    //if(block == 1000) printf("\nblock %d, distance = %d", blockIdx.x, windowMap[windowMapBlockIndex+2]);
}
...
void BM_CalculateDistance(int* windowMap, int size, int windowSize, int width, float* image, double* blocks, int blockSize)
{
    ...
    int block = (blockIdx.y * size) + blockIdx.z;
    int blockIndex = (block * blockSize) + 64 + (blockIdx.x << 6) + (threadIdx.x << 3);
...
{
    int windowMapIndex = (((blockIdx.y * size) + blockIdx.z) * windowSize) + 4 + (blockIdx.x * 4);
...
void BM_Sort(int* windowMap, int size, int windowSize, int nbBlocksPerWindow, int maxBlock, int* nbSimilarBlocks)
{
    ...
    for(int n = 0; n< maxBlock; n++)
    {
        ...
        {                
            if(windowMap[windowMapIndex + i * 4 + 2] > -1  && windowMap[windowMapIndex + i * 4 + 2] < currentDistance && windowMap[windowMapIndex + i * 4 + 2] > 0) { foundIndex = i; currentDistance = windowMap[windowMapIndex + i * 4 + 2]; }
        }   
        if(foundIndex > -1)
        {
...
void BM3D::BM3D_BlockMatching(bool final)
{
    ...
    {
        dim3 numBlocks(BM3D::context.nbBlocksPerWindow, BM3D::context.widthBlocksIntern, BM3D::context.widthBlocksIntern);
        dim3 numThreads(1);
...
{
    int block = (blockIdx.y * size) + blockIdx.z;
    int windowMapIndex = (block * windowSize) + (blockIdx.x * 4);
    if(windowMap[windowMapIndex+3] < similarBlock[block] && windowMap[windowMapIndex+3] > -1)
    { 
        int block3DIndex = (((blockIdx.y * size) + blockIdx.z) * 2112) + (windowMap[windowMapIndex+3] * 66) +  (threadIdx.y << 3) + threadIdx.x;
...
void AddBlockPosition(double* blocks3D, int* windowMap, int size, int windowSize, int* similarBlock)
{
    ...
    if(windowMap[windowMapIndex+3] < similarBlock[block] && windowMap[windowMapIndex+3] > -1)
    { 
        int block3DIndex = (((blockIdx.y * size) + blockIdx.z) * 2112) + (windowMap[windowMapIndex+3] * 66);
...
void CreateWindowBlocks(int* windowMap, int widthBlockIntern, int p, int windowSize, int offset, int halfWindowSize, int widthBlocksWindow)
{
    ...
    int blockY = refY - halfWindowSize + blockIdx.z * p;
    int windowBlockIndex = windowMapIndex + 4 + (((blockIdx.z * widthBlocksWindow) + threadIdx.x) * 4);
...
void BM3D::BM3D_CreateWindow(bool final)
{
    ...
    {
        dim3 numBlocks(BM3D::context.widthBlocksIntern, BM3D::context.widthBlocksIntern, BM3D::context.widthBlocksWindow);
        dim3 numThreads(BM3D::context.widthBlocksWindow);
        CreateWindowBlocks<<<numBlocks,numThreads>>>(BM3D::context.windowMap, BM3D::context.widthBlocksIntern, BM3D::context.stepInWindow, BM3D::context.windowSize, BM3D::context.offset, BM3D::context.halfWindowSize, BM3D::context.widthBlocksWindow); 
        cudaDeviceSynchronize();   
    }  
}	
