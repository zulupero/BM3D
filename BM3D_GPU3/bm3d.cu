...
void HardThresholdFilter(double* blocks3D, double threshold, int size, int* nbSimilarBlocks)
{
    ...
    //if(block == 6670) printf("\nblock = %d, coef = %d", block, nbSimilarBlocks[block]);
}
...
void AddBlockPosition(double* blocks3D, int* windowMap, int size, int windowSize, int limit, int* similarBlock)
{
    ...
    { 
        ...
        //if((blockIdx.y * size) + blockIdx.x == 6670) printf("\nx = %f, y = %f", blocks3D[block3DIndex+64], blocks3D[block3DIndex+65]);
