...
void BM3D::BM3D_Initialize(BM3D::SourceImage img, int width, int height, int pHard, int hardLimit, double hardThreshold, int sigma, bool debug)
{
    ...
    gpuErrchk(cudaMalloc(&BM3D::context.deviceImage, w2 * h2 * sizeof(float)));
    gpuErrchk(cudaMemcpy(BM3D::context.deviceImage, &img[0], w2 * h2 * sizeof(float), cudaMemcpyHostToDevice));
    ...
    printf("\n\tSize Image array          = %f Mb", (w2 * h2 * sizeof(float)/1024.00 / 1024.00));  
    printf("\n\tBasic Image array         = %f Mb", (w2 * h2 * sizeof(float)/1024.00 / 1024.00));
...
void pre_aggregation(double* blocks3D, double* wpArray, double* blocks, int* bmVectors, int size, float* kaiserCoef, float* estimates, int width)
{
    ...
    if(block == 84) printf("\nblock = %d, xPixel = %d, yPixel =%d, num = %f, den = %f, wp = %f, coef = %f, value = %f", block, xPixel, yPixel, (kaiserCoef[kaiserIndex] * wpArray[block] * blocks3D[block3DIndex]), (kaiserCoef[kaiserIndex] * wpArray[block]), wpArray[block], kaiserCoef[kaiserIndex], blocks3D[block3DIndex]);
}
...
__global__
void HardThresholdFilter(double* blocks3D, double threshold, int size)
{
    int blockPixelIndex = (((blockIdx.y * size) + blockIdx.x) << 6) + (threadIdx.y << 3) + threadIdx.x;
    if(blocks3D[blockPixelIndex] < threshold) blocks3D[blockPixelIndex] = 0;  
}
...
{
    {
        dim3 numBlocks(BM3D::context.widthBlocksIntern << 2, BM3D::context.widthBlocksIntern << 2);
        dim3 numThreads(8, 8);
        HardThresholdFilter<<<numBlocks,numThreads>>>(BM3D::context.blocks3D, BM3D::context.hardThreshold, BM3D::context.widthBlocksIntern << 2); 
        cudaDeviceSynchronize();   
    }
    {
        dim3 numBlocks(BM3D::context.widthBlocksIntern, BM3D::context.widthBlocksIntern);
...
__global__
void ShiftImage(float* originalImage, int widthOrig, int width, int offset)
{
    int x = blockIdx.x * blockDim.x + threadIdx.x;
    int y = blockIdx.y * blockDim.y + threadIdx.y;
    int originalImageIndex = (y * widthOrig) + x;
    int basicImageIndex = ((y+offset) * width) + (x+offset);
    originalImage[basicImageIndex] = originalImage[originalImageIndex];
}

__global__
void CreateBlocks(float* deviceImage, double* blocks, int size, int width, int pHard)
{
    ...
    blocks[blockPixelIndex] = deviceImage[imgIndex];  
}
...
void BM3D::BM3D_CreateBlock()
{
    int offset = 5 * BM3D::context.pHard;
    {
        ...
        dim3 numThreads(8,8);
        ShiftImage<<<numBlocks,numThreads>>>(BM3D::context.deviceImage, BM3D::context.img_widthOrig, BM3D::context.img_width, offset); 
        cudaDeviceSynchronize();   
    }
    {
        dim3 numBlocks(BM3D::context.widthBlocks, BM3D::context.widthBlocks);
        dim3 numThreads(8,8);
        CreateBlocks<<<numBlocks,numThreads>>>(BM3D::context.deviceImage, BM3D::context.blocks, BM3D::context.widthBlocks, BM3D::context.img_width, BM3D::context.pHard); 
        cudaDeviceSynchronize();   
    }
    {
        dim3 numBlocks(BM3D::context.widthBlocks, BM3D::context.widthBlocks);
