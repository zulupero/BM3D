...
void WienFilter(double* blocks3D, double* blocks3DOrig, int size, float* similarBlocks, int sigma, float* wpArray)
{
    ...
    int estimateValue = blocks3D[blockPixelIndex];
    float value = estimateValue * estimateValue;// * similarBlocks[block];
    value /= (value + (sigma * sigma));
    blocks3D[blockPixelIndex] = blocks3DOrig[blockPixelIndex] * value;// * similarBlocks[block];
...
{
    similarBlocks[(blockIdx.y * size) + blockIdx.x] = 1.0f/similarBlocks[(blockIdx.y * size) + blockIdx.x];
}

__global__
void CalculateCoefSimilarBlocksBasicEstimate(float* similarBlocks, int size)
{
...
void BM3D::BM3D_BlockMatching(bool final)
{
    ...
    }
    {
        ...
        else
        {
            dim3 numBlocks(BM3D::context.widthBlocksIntern, BM3D::context.widthBlocksIntern);
            dim3 numThreads(1);
            CalculateCoefSimilarBlocksBasicEstimate<<<numBlocks,numThreads>>>(BM3D::context.nbSimilarBlocks, BM3D::context.widthBlocksIntern); 
            cudaDeviceSynchronize();
        }
    }
    {
